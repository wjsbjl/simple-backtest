...
class Displayer(object):
    ...
    def _calc_holding_infos(self): # TODO
        ...
        holding_infos['returns'] = holding_infos.apply(lambda x:
                                                       ...
                                                       axis=1)
        # self.df.columns
        # self.df.loc[self.df.position !=0,['position', 'num']]
        # self.df.loc[:,'num']=list(range(len(self.df)))]
...
class BackTester(object):
    ...
    def construct_position_(self,
        ...
        """
        assert 'signal' in self.backtest_env.columns, '未计算信号'
        self.backtest_env['position'] = self.backtest_env['signal'].shift(1) # TODO:这里直接就持有了,加个shift1
